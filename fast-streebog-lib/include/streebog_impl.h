/**
 * @file streebog_impl.h
 * @brief C implementations of all Streebog transforms
 *
 * These are portable C implementations that can be used instead of ASM versions.
 * Control which implementation to use via STREEBOG_USE_ASM define:
 *   - Define STREEBOG_USE_ASM=1 (or just STREEBOG_USE_ASM) to use ASM implementations
 *   - Define STREEBOG_USE_ASM=0 or don't define it to use C implementations
 *
 * On MSVC with x64, ASM is used by default. On other platforms, C is used.
 */

#ifndef STREEBOG_IMPL_H
#define STREEBOG_IMPL_H
...
// ==================== TAU permutation for P-transform ====================
static const uint8_t STREEBOG_TAU[64] = {0, 8,  16, 24, 32, 40, 48, 56, 1, 9,  17, 25, 33, 41, 49, 57,
...
// ==================== Round constants for KeySchedule ====================
...
static inline void STREEBOG_NAMESPACE(l_transform_c_inline)(const uint8_t *state, uint8_t *out)
{
    for (int i = 0; i < 8; i++)
    {
        ...
        // Use precalculated tables: result = Ax[0][byte0] ^ Ax[1][byte1] ^ ... ^ Ax[7][byte7]
        ...
        // Write result in big-endian format
        streebog_u64_to_bytes_be(result, out + i * 8);
    }
}
...
static inline void streebog_g_n_c(const uint8_t *N, const uint8_t *h, const uint8_t *m, uint8_t *out)
{
    uint8_t K[64];
    uint8_t t[64];
