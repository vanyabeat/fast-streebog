...
static void ref_s(const uint8_t* in, uint8_t* out) { streebog_s_transform_c(in, out); }
static void ref_p(const uint8_t* in, uint8_t* out) { streebog_p_transform_c(in, out); }
static void ref_l(const uint8_t* in, uint8_t* out) { streebog_l_transform_c(in, out); }
static void ref_xor(const uint8_t* a, const uint8_t* b, uint8_t* out) { streebog_xor_512_c(a, b, out); }
static void ref_add(const uint8_t* a, const uint8_t* b, uint8_t* out) { streebog_add_512_c(a, b, out); }
static void ref_ks(const uint8_t* k, int i, uint8_t* out) { streebog_key_schedule_c(k, i, out); }

struct Impl {
    const char* name;
    int (*supported)();
    void (*s)(const uint8_t*, uint8_t*);
    void (*p)(const uint8_t*, uint8_t*);
    void (*l)(const uint8_t*, uint8_t*);
    void (*xor_fn)(const uint8_t*, const uint8_t*, uint8_t*);
    void (*add_fn)(const uint8_t*, const uint8_t*, uint8_t*);
    void (*ks)(const uint8_t*, int, uint8_t*);
};
...
TEST(DispatchCorrectness, STransform)
{
    ...
    {
        if (!impl.supported()) {
            std::cout << "[SKIP] " << impl.name << " S-transform (not supported)\n";
            continue;
        }

        for (int offset = 0; offset < 4; offset++)
        {
            std::vector<uint8_t> in(64), ref(64), out(64);
            for (int i = 0; i < 64; i++)
                in[i] = static_cast<uint8_t>(offset * 64 + i);

            ref_s(in.data(), ref.data());
            impl.s(in.data(), out.data());

            EXPECT_EQ(out, ref)
                << impl.name << " S-transform failed at offset " << offset;
        }
    }
}

TEST(DispatchCorrectness, PTransform)
{
    for (const auto& impl : IMPLS)
    {
        if (!impl.supported()) {
            std::cout << "[SKIP] " << impl.name << " P-transform (not supported)\n";
            continue;
        }
        ...
        ref_p(in.data(), ref.data());
        impl.p(in.data(), out.data());

        EXPECT_EQ(out, ref) << impl.name << " P-transform failed";
    }
}

TEST(DispatchCorrectness, LTransform)
{
    for (const auto& impl : IMPLS)
    {
        if (!impl.supported()) {
            std::cout << "[SKIP] " << impl.name << " L-transform (not supported)\n";
            continue;
        }
        ...
        ref_l(in.data(), ref.data());
        impl.l(in.data(), out.data());

        EXPECT_EQ(out, ref) << impl.name << " L-transform failed";
    }
}

TEST(DispatchCorrectness, XOR512)
{
    for (const auto& impl : IMPLS)
    {
        if (!impl.supported()) {
            std::cout << "[SKIP] " << impl.name << " XOR-512 (not supported)\n";
            continue;
        }

        std::vector<uint8_t> a(64), b(64), ref(64), out(64);
        for (int i = 0; i < 64; i++) { a[i] = static_cast<uint8_t>(i); b[i] = static_cast<uint8_t>(~i); }

        ref_xor(a.data(), b.data(), ref.data());
        impl.xor_fn(a.data(), b.data(), out.data());

        EXPECT_EQ(out, ref) << impl.name << " XOR-512 failed";
    }
}

TEST(DispatchCorrectness, ADD512)
{
    for (const auto& impl : IMPLS)
    {
        if (!impl.supported()) {
            std::cout << "[SKIP] " << impl.name << " ADD-512 (not supported)\n";
            continue;
        }

        std::vector<uint8_t> a(64, 0), b(64, 0), ref(64), out(64);
        a[63] = 0xFF; b[63] = 0x01; // carry test

        ref_add(a.data(), b.data(), ref.data());
        impl.add_fn(a.data(), b.data(), out.data());

        EXPECT_EQ(out, ref) << impl.name << " ADD-512 failed";
    }
}

TEST(DispatchCorrectness, KeySchedule)
{
    for (const auto& impl : IMPLS)
    {
        if (!impl.supported()) {
            std::cout << "[SKIP] " << impl.name << " KeySchedule (not supported)\n";
            continue;
        }

        std::vector<uint8_t> k(64), ref(64), out(64);
        for (int i = 0; i < 64; i++)
            k[i] = static_cast<uint8_t>(i);

        for (int round = 0; round < 12; round++)
        {
            ref_ks(k.data(), round, ref.data());
            impl.ks(k.data(), round, out.data());

            EXPECT_EQ(out, ref)
                << impl.name << " KeySchedule failed at round " << round;
        }
    }
}
